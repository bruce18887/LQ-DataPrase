...
- 2026-09-13 回退：§1.3 Fail/超界 独立置顶强调层按用户反馈撤销（并集层名易被误读为
  「红点=超界」：带内红点实为跨测试项 fail 的 die）；fail/超界点恢复随 Site 系列着色
  （自适应点径/透明度、最密垫底 z 序同样生效）。§1.1/§1.2、§2、§3 保持生效；
  §4 errorColor 断言相应移除，双主题覆盖由视觉快照保留
