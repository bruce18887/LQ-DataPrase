...
/**
 * 自适应精度百分比显示（2026-08-30 用户定稿：最多 3 位小数）：
 * 默认 2 位小数（0.01% 精度）；仅当值小于 0.01% 时扩展到 3 位；
 * 更小的非零值显示 "<0.001"，避免固定 toFixed 造成的假零。
 * - 0.12 → "0.12"  0.05 → "0.05"  0.005 → "0.005"  0.0005 → "<0.001"
 */
export function formatPercent(v: number): string {
  if (v <= 0) return '0'
  if (v < 0.001) return '<0.001'
  let decimals = 2
  while (v < 10 ** -decimals && decimals < 3) decimals++
