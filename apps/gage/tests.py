...
class GenerateSummaryFilenameTests(APITestCase):
    """POST /api/v1/gage/generate_summary/ 文件名模板。"""

    def setUp(self):
        self.user = User.objects.create_user(username='gageuser', password='pw')
    ...
    def test_default_template_contains_datetime(self):
        ...
        self.assertIsNotNone(
            re.search(r'Gage_Summary_\d{8}_\d{6}\.xlsx', _filename_from_cd(cd)),
            f'默认模板应含日期时间戳: {cd}',
        )
    ...
    def test_custom_template_with_file_count(self):
        ...
        self.assertEqual(_filename_from_cd(resp['Content-Disposition']), 'GS_2f.xlsx')
